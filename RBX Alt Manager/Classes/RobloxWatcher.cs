    ...
    internal class RobloxWatcher
    {
        ...
        public static void CheckProcesses()
        {
            ...
            foreach (var process in Process.GetProcessesByName("RobloxPlayerBeta"))
            {
                ...
                try { CommandLine = process.GetCommandLine(); } catch { continue; } // MULTI-INSTANCE FIX: Gracefully handle access denied
                ...
                try
                {
                    // MULTI-INSTANCE FIX: Increased grace period from 30s to 90s
                    // Multiple instances take longer to fully start up
                    if ((DateTime.Now - process.StartTime).TotalSeconds > 90)
                    {

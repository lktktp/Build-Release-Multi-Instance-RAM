...
namespace RBX_Alt_Manager
{
    public partial class AccountManager : Form
    {
        ...
        public AccountManager()
        {
            ...
            // MULTI-INSTANCE FIX: Enable multi-roblox by default
        ...
        /// <summary>
        /// Temporarily release the singleton mutex so a new Roblox instance can start.
        /// Called by Account.JoinServer before launching RobloxPlayerBeta.exe.
        /// </summary>
        public void ReleaseMutexForLaunch()
        {
            if (rbxMultiMutex != null)
            {
                try
                {
                    rbxMultiMutex.ReleaseMutex();
                    Program.Logger.Info("Released ROBLOX_singletonMutex for launch");
                }
                catch (Exception ex)
                {
                    Program.Logger.Error($"Failed to release mutex for launch: {ex.Message}");
                }
            }
        }

        /// <summary>
        /// Re-acquire the singleton mutex after a Roblox instance has started.
        /// This prevents subsequent Roblox instances from being blocked by the singleton check.
        /// </summary>
        public void ReacquireMutexAfterLaunch()
        {
            if (rbxMultiMutex != null)
            {
                try
                {
                    rbxMultiMutex.WaitOne(TimeSpan.FromSeconds(5), true);
                    Program.Logger.Info("Re-acquired ROBLOX_singletonMutex after launch");
                }
                catch (Exception ex)
                {
                    Program.Logger.Error($"Failed to re-acquire mutex after launch: {ex.Message}");
                    // Try to recreate the mutex
                    try
                    {
                        rbxMultiMutex.Close();
                        rbxMultiMutex = new Mutex(true, "ROBLOX_singletonMutex");
                        Program.Logger.Info("Recreated ROBLOX_singletonMutex after failed re-acquire");
                    }
                    catch (Exception ex2)
                    {
                        Program.Logger.Error($"Failed to recreate mutex: {ex2.Message}");
                    }
                }
            }
            else if (General.Get<bool>("EnableMultiRbx"))
            {
                // Mutex was null but should be enabled - recreate it
                try
                {
                    rbxMultiMutex = new Mutex(true, "ROBLOX_singletonMutex");
                    Program.Logger.Info("Created new ROBLOX_singletonMutex (was null)");
                }
                catch (Exception ex)
                {
                    Program.Logger.Error($"Failed to create new mutex: {ex.Message}");
                }
            }
        }
